...
mod tests {
    ...
    #[test]
    fn disabled_live_services_can_stop_but_cannot_restart() {
        for state in [
            ...
            let row = to_service_row(&disabled, true);
            assert!(row.can_stop, "Stop must remain available for {state:?}");
            assert!(
                !row.can_restart,
                "Restart must respect disabled startup even while {state:?}"
            );
        }
    }
